...
class mscreenclass():
    ...
    def mscreenmethod(welcome):
        ...
        searchentry = ttk.Combobox(root,postcommand=lambda: searchentry.configure(values=mscreenclass.changeMonth(1)))
        searchentry.current()
        myFont = font.Font(family='Courier', size=10, weight='bold')

        cbutton = Button(root, text="CLEAR",bg='#BCC6CE',padx=10,pady=10, command=lambda: mscreenclass.changeMonth(0))
        cbutton['font']=myFont
...
class searchclass:
  ...
  def searchmethod(search_text):
        ...
        #elif('songs' in search_text or 'music' in search_text):
            #webbrowser.open()
